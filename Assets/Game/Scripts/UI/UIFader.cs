...
public class UIFader : MonoBehaviour
{
    ...
    public void Fade(FADE fadeDir,float fadeDuration, float StartDelay)
    {
        if (img != null)
        {
            if (fadeDir == FADE.FadeIn) StartCoroutine(FadeCoroutine(1f, 0f, fadeDuration, StartDelay, true));
        }
            if (fadeDir == FADE.FadeOut) StartCoroutine(FadeCoroutine(0f, 1f, fadeDuration, StartDelay, false));
    }
